...
% \paragraph{Demand validation.} Respondents unanimously rated both the overall value of such a resource and the ability to search across multiple sources at M=4.62 on a 5-point scale, directly validating LACK's integration approach.
...
\paragraph{Priority relation types.} Funding flows received the highest mean rating (M=4.75), followed by employment (M=4.50), and membership, sponsorship, and founding (each M=4.38).
...
\paragraph{Use cases and the political sphere.} The most frequently selected CQs concerned funding chains (5/9), revolving doors between industry and government (4/9), and multi-hop connections between corporations and advocacy groups (4/9), the latter aligning well with LACK's demonstrated network reach (CQ9, 93.5\%).
