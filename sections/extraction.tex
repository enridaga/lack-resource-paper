\begin{figure}[t]
  \centering
  \includegraphics[width=\textwidth]{figures/lack-acquisition-pipeline.pdf}\vspace{-0.8cm}
...
CQ1--CQ11 are listed in Table~\ref{tab:coverage} and drove the design of the ontology (Section~\ref{sec:ontology}) and pipeline; CQ12 (identity and deduplication) is addressed by entity linking, below.
The full text is on the LACK website\footnote{\url{https://climatesense-project.eu/lack/ontology.html}}.
%
...
The pipeline (implemented in a Google Colab notebook, deposited~\cite{lack-re}) processes one page at a time with resumption and re-runs failed pages until all are successfully processed.
The output is one CSV per source, with a row per relation containing source, target, their types, relation type, temporal annotations, a natural-language description, and the evidence URL.
%
%
...
We evaluate source coverage, relation accuracy, entity-linking accuracy, and known limitations.
All annotators are knowledge engineers; both tasks below were preceded by a sample run to align on the assessment dimensions.

Our collection covers DeSmog Climate Disinformation Database and the LobbyMap Scores leaderboard in their entirety at the time of collection.
InfluenceMap hosts further content (sector-level reports, country profiles, and additional policy engagement assessments) not included here, which we leave to future work.
...
Partly correct answers mostly stem from sub-role conflation (e.g., \texttt{is president of} used for vice-presidents).
A recurrent validity issue is the misapplication of organisation-level relations to person-organisation pairs (e.g., \texttt{has partner} used for a partner title at a law firm rather than a partnership between two organisations).
Temporal correctness is the least accurate dimension (~0.80\%), with errors from missing dates, incorrect inference of date ranges, and ambiguity in encoding point-in-time vs.\ interval information.
...
\begin{table}[t]
\centering
\caption{Mapping of competency questions to ontology relations and triple counts. Coverage ratio and tier discussion in Section~\ref{sec:evaluation}}
